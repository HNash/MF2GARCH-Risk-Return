...
def hessianTwoSided(llfunc, params, y, cond_means, m):
    n = params.size

    fx = llfunc(params, y, cond_means, m)
    ...
    gm = np.zeros(n)
    for i in range (n):
        gp[i] = llfunc(params+ee[:,i].toarray().ravel(), y, cond_means, m)
        gm[i] = llfunc(params-ee[:,i].toarray().ravel(), y, cond_means, m)
    ...
    for i in range(n):
        for j in range(n):
            Hp[i,j] = llfunc(params+ee[:,i].toarray().ravel()+ee[:,j].toarray().ravel(), y, cond_means, m)
            Hp[j,i] = Hp[i,j]
            Hm[i,j] = llfunc(params-ee[:,i].toarray().ravel()-ee[:,j].toarray().ravel(), y, cond_means, m)
            Hm[j,i] = Hm[i,j]
...
def stdErrors(params, y, cond_means, e, h, tau, m):
    T = tau.size
    k = params.size
    ...
    for j in range(k):
        ...
        ll_rr_h = -0.5 * (np.log(2*np.pi*(np.multiply(h_h, tau_h))) + np.divide(np.power(np.subtract(cond_means[505:],(gamma_0_h+gamma_1_h*tau_h)),2), np.multiply(h_h, tau_h)))
        ...
        ll_rr_m = -0.5 * (np.log(2*np.pi*(np.multiply(h_m, tau_m))) + np.divide(np.power(np.subtract(cond_means[505:],(gamma_0_m+gamma_1_m*tau_m)),2), np.multiply(h_m, tau_m)))
    ...
    # Hessian
    H = hessianTwoSided(estimation.totallikelihood,params, y, cond_means, m)
    A = H/T
